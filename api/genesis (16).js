...
- nova : génération d'image, retouche photo, logo, cartoon (UNIQUEMENT si l'utilisateur demande explicitement de créer, générer ou modifier une image — PAS si il envoie une photo pour l'analyser ou poser une question)
...
Réponds UNIQUEMENT avec ce JSON valide, sans backticks :
{
  ...
  "suggested_expert": null
    ...
    // ── Routing expert Nexo ───────────────────────────────────────────────
    // Si le message contient une image, ne jamais rediriger vers Nova (l'utilisateur veut analyser, pas générer)
    const hasImage = messages.some(m => Array.isArray(m.content) && m.content.some(c => c.type === 'image'));
    const blockNovaRedirect = hasImage && dec.expertId === 'nova';
    if (isFirstMessage && dec.route === 'redirect' && dec.confidence > 0.75 && NEXO_EXPERTS.includes(dec.expertId) && !blockNovaRedirect) {
      return res.status(200).json({ redirect: { expertId: dec.expertId, reason: dec.reason } });
    }
    if (isFirstMessage && dec.route === 'ambiguous' && NEXO_EXPERTS.includes(dec.expertId) && NEXO_EXPERTS.includes(dec.expertId2) && !blockNovaRedirect) {
    ...
    async function callGPT() {
      ...
      const r = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: 'gpt-4o-search-preview', max_tokens: 1500, messages: oaiMsgs, web_search_options: { search_context_size: 'medium' } })
      });
      const d = await r.json();
    ...
    if (finalModels.length === 1) {
      ...
      // Fallback chain: si Gemini échoue (quota), Claude a aussi le web search
      if (!text && m === 'gemini') text = await callClaude();
      if (!text) text = await callGPT() || await callClaude();
      if (!text) return res.status(500).json({ error: 'Aucune réponse disponible' });
    // ── Extraction mémoire asynchrone (comme ChatGPT) ────────────────────
    // Lance l'analyse en arrière-plan SANS bloquer la réponse à l'utilisateur
    extractAndSaveMemory(userText, userEmail, firstName, memorySummary, SUPABASE_URL, SUPABASE_SERVICE, ANTHROPIC_KEY);
    ...
    await Promise.all(Object.entries(calls).map(async ([name, promise]) => {
      ...
      // Si Gemini échoue en fusion, Claude prend le relais (a aussi le web search)
      if (name === 'gemini' && !results[name] && !results['claude']) {
        try { results['claude'] = await callClaude(); } catch(e) {}
      }
    }));
    ...
    let fusedText = null;    if (bestSynth === 'gpt' && OPENAI_KEY) {
      const r = await fetch('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${OPENAI_KEY}` },
...
// ── Extraction et sauvegarde mémoire (asynchrone, ne bloque jamais la réponse) ──
// Fonctionne comme ChatGPT : détecte le prénom, les infos importantes mentionnées
// par l'utilisateur et met à jour le profil Supabase en arrière-plan.
async function extractAndSaveMemory(userText, userEmail, currentFirstName, currentSummary, SUPABASE_URL, SUPABASE_SERVICE, ANTHROPIC_KEY) {
  try {
    const prompt = `Tu es un système de mémoire pour une IA personnelle, comme ChatGPT Memory.

Message de l'utilisateur : "${userText}"

Résumé mémoire actuel : ${currentSummary || 'Aucun'}
Prénom connu : ${currentFirstName || 'Inconnu'}

Analyse le message et détermine :
1. L'utilisateur mentionne-t-il son prénom ? Si oui, lequel ?
2. Y a-t-il une information importante à retenir (métier, projet, situation familiale, préférence, contexte de vie) ?
...
- "first_name" : le prénom si mentionné, sinon null
- "new_info" : une phrase courte (max 20 mots) résumant l'info importante, sinon null  
- "should_update" : true si au moins une des deux est non-null

Si le message est une question banale sans info personnelle, should_update = false.`;
    ...
    const d = await r.json();
    const raw = (d.content?.[0]?.text || '').trim().replace(/^```json\s*|```\s*$/g, '');
    const result = JSON.parse(raw);

    if (!result.should_update) return;

    const updates = {};
    if (result.first_name && !currentFirstName) updates.first_name = result.first_name;
    if (result.new_info) {
      const existing = currentSummary || '';
      updates.memory_summary = existing ? `${existing}. ${result.new_info}` : result.new_info;
    ...
    // Silencieux — la mémoire ne doit jamais faire planter Genesis
